...
abs(e.salary - AVG(e.salary) OVER (PARTITION BY e.department_id)) AS diff_from_avg
FROM employees e
JOIN departments d
ON e.department_id = d.id;
...
select first_name,last_name,name,salary from ranked_employees where rank<=2;
...
with recursive management_chain as(
    select e.id,e.first_name,e.last_name,e.manager_id,1 as level
    from employees e
    where e.id=3
    union all
    select e.id,e.first_name,e.last_name,e.manager_id,mc.level+1
    from employees e
...
--Solution:
select name as Project_name,budget,round(budget*100/sum(budget) over(),2) as budget_percentage
from projects;
...
with sales_sumarray as(
    select e.id,e.first_name,e.last_name,s.region,sum(s.amount) as total_sales
    from employees e join sales s
    on e.id=s.employee_id
    group by e.id,e.first_name,e.last_name,s.region
)
select first_name,last_name,region,total_sales,
...
--Solution:
select d.name as department_name,count(distinct e.id) as total_headcount,sum(e.salary) as total_salary_budget,
...
with salary_stats as(
    select avg(salary) as avg_salary,sum(salary) as total_salary from employees
)
select e.first_name,e.last_name,e.salary,round(e.salary*100/total_salary,2) as salary_percentage
from employees e cross join salary_stats s
...
--Solution:
select e.first_name,e.last_name,min(s.sale_date) as first_sale,max(s.sale_date) as last_sale
from employees e join sales s 
on e.id=s.employee_id
...
--Solution:
select e.first_name as emp1_first_name,e.last_name as emp1_last_name,e2.first_name as emp2_first_name,e2.last_name as emp2_last_name, d.name as department_name, 
e.hire_date as emp1_hire_date,e2.hire_date as emp2_hire_date
from employees e join employees e2
on e.department_id=e2.department_id and e.id<e2.id
...
--Solution:
select distinct e.first_name,e.last_name,p.name as project_name,p.end_date
from employees e
...
with project_hours as (
    select employee_id,count(distinct project_id) as num_projects,sum(hours_logged) as total_hours
...
select e.first_name||' '||e.last_name as full_name,d.name as department_name,
coalesce(m.first_name||' '||m.last_name,'No Manager') as manager_name,
e.salary,
case    
when e.salary<75000 then 'Junior'
when e.salary between 75000 and 99999 then 'Mid'
else 'Senior' end as salary_band,
